...
- **`datahub_entity_ownership`**, a new resource that assigns owners to the platform-configuration entities this provider manages. One resource per owned entity, holding a set of `owner` entries, each one an `(owner, ownership type)` pair. It completes `datahub_ownership_type`, which until now could define a vocabulary of roles that nothing in the provider could use.
  ...
  **Import adopts every owner on the entity, including ones DataHub assigned itself.** Creating a glossary term makes the creating actor a Technical Owner automatically (`OwnerUtils.addCreatorAsOwner`), so an entity nobody has curated by hand still arrives with one owner, and which actor it is depends on the instance -- an OSS Quickstart runs with metadata service auth off and attributes everything to the system actor. So after importing, write the configuration from `terraform state show` rather than from memory: an adopted owner left out of it is one the next apply removes. Expect a non-empty plan straight after an import for the same reason.
...
- **Destroying a Cloud assertion no longer risks orphaning its monitor.** Every monitor-backed assertion resource (`datahub_freshness_assertion`, `datahub_volume_assertion`, `datahub_sql_assertion`, `datahub_field_assertion`, `datahub_schema_assertion`) now records a computed `monitor_urn` at create time and refreshes it on read, so destroy deletes the monitor through a reference it already holds instead of re-resolving it through an eventually-consistent graph query at the moment of deletion. Previously a failed lookup -- index lag was enough -- was silently discarded: the assertion was deleted, the monitor stayed behind, and nothing reported it. Orphaned monitors count toward a DataHub Cloud tenant's monitor limit and block recreating an assertion of the same type on the same dataset, so the leak surfaced later as an unrelated-looking apply failure.
  ...
  A refresh cannot undo the protection: when the monitor lookup returns nothing without an error -- which index lag produces just as readily as genuine absence -- Read keeps the stored `monitor_urn` rather than clearing it. Clearing it would have sent the next destroy back into the same eventually-consistent lookup through a window one refresh wide. A preserved-but-stale URN costs nothing, because deleting an already-absent monitor counts as success.
...
- **New `datahub_form` resource and `datahub_forms` data source** manage DataHub metadata forms -- the questionnaires (`COMPLETION`) and compliance attestations (`VERIFICATION`) whose prompts ask asset owners to fill in structured property values. The rule-based assignment (`dynamicFormAssignment`) is a nested `dynamic_assignment` block on the resource rather than a separate resource, because it is an aspect of the form itself: at most one per form, meaningless without it, and deleted with it. Works on both OSS DataHub and DataHub Cloud; the caller needs the Manage Metadata Forms platform privilege.

  The URN is `urn:li:form:<form_id>`, matching the Python SDK convention, so Terraform-managed and SDK-managed forms converge on the same entity rather than duplicating. Prompt ids are required in configuration for the same reason: the server otherwise mints a random UUID per prompt, and prompt completion records are keyed by the id, so a nondeterministic one would both drift state and orphan responses.
