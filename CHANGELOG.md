...
- `datahub-tf-import` CLI: `enumerate` command bulk-imports a brownfield DataHub deployment by enumerating all resources of each registered type, writing `import {}` blocks, driving `terraform plan -generate-config-out`, and post-processing the output to insert `var.*` references for WriteOnly attributes (secrets) and platform-block stubs for connections. Eliminates the need to hand-author hundreds of resource blocks and hunt down URNs manually.
