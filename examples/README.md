...
- `runnable/form-simple/` -- stand up the two form types side by side: a `COMPLETION` form collecting two structured property values from asset owners and a stewardship group, and a `VERIFICATION` form asking owners to attest the recorded value, each dynamically assigned to matching datasets
- `runnable/metadata-test-simple/` -- define two declarative governance rules (every dataset has a description; PROD datasets have an owner) and enumerate every metadata test on the instance as a bulk-import source; the whole CRUD surface works on OSS, but only DataHub Cloud evaluates the definitions and the OSS UI hides them behind `testsConfig.enabled`
...
- `runnable/data-contract-with-ingestion/` -- attach a data contract and a custom assertion to a dataset this configuration ingests itself, triggering the ingestion run through a `local-exec` provisioner because running a source is a verb the provider has no resource for; needs `curl` and `bash`, plus a running ingestion executor, and the ingested dataset is unmanaged so `terraform destroy` leaves it behind
- `runnable/local-iam/` -- set up local-auth users, group membership, a role assignment, and an access policy for a team
- `runnable/page-template-simple/` -- the minimal `datahub_page_module` and `datahub_page_template` pair: two modules and one template under its own id, which nothing points at, so it changes nothing your users see
- `runnable/home-page-layout/` -- build the organisation's home page from Terraform: four modules arranged into three rows by adopting the instance's default template, so a rebuilt instance comes back with its landing page intact; **applying replaces the page every user sees except those who have personalised their own, and `terraform destroy` restores the previous layout rather than deleting the page**
...
- `runnable/oauth-authorization-server-simple/` -- register an outbound OAuth client configuration DataHub uses to obtain tokens from an external identity provider on a user's behalf, which an Ask DataHub AI plugin in `USER_OAUTH` mode then references by URN; the client secret is supplied as a WriteOnly attribute, so Terraform 1.11 or later is required (DataHub Cloud only)
- `runnable/org-branding-simple/` -- set the organization name, logo and brand colour every user of the instance sees, and record the previous branding through the matching data source first; **applying replaces existing branding for everyone and `terraform destroy` resets it to DataHub's defaults rather than restoring it** (DataHub Cloud only; `primary_color` needs Cloud v2.2.0 or later)
