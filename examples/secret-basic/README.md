...
2. `datahub_ingestion_source.demo` contains `"${demo-api-token}"` in the recipe JSON.
...
The URN (`urn:li:dataHubSecret:demo-api-token`) is unchanged after rotation because the name stays the same, so the recipe reference `${demo-api-token}` continues to work without any recipe edit.
...
```bash
terraform import datahub_secret.demo_token urn:li:dataHubSecret:demo-api-token
# or equivalently:
terraform import datahub_secret.demo_token demo-api-token
```
