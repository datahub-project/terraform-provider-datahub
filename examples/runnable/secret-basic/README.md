...
1. `datahub_secret.example_secret` stores the plaintext under an encrypted form in DataHub. The value is never written to `terraform.tfstate`.
2. `datahub_ingestion_source.example` contains `"${TF_EXAMPLE_SECRET_BASIC}"` in the recipe JSON.
...
3. Run `terraform apply` -- Terraform plans a replacement (delete + create) of `datahub_secret.example_secret`.

The URN (`urn:li:dataHubSecret:TF_EXAMPLE_SECRET_BASIC`) is unchanged after rotation because the name stays the same, so the recipe reference `${TF_EXAMPLE_SECRET_BASIC}` continues to work without any recipe edit.
...
```bash
terraform import datahub_secret.example_secret urn:li:dataHubSecret:TF_EXAMPLE_SECRET_BASIC
# or equivalently:
terraform import datahub_secret.example_secret TF_EXAMPLE_SECRET_BASIC
```
