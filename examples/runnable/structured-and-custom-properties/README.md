...
1. **Avoid the situation.** For a schema change, bump the property's `version` rather than destroying and recreating it. Same URN and `qualifiedName`; search moves to a new versioned field, and the old field name is never burned. (DataHub derives a different ES field name entirely once `version` is set, which is what makes this work.) The provider exposes this as the optional `version` attribute on `datahub_structured_property`; set it to a 14-digit `yyyyMMddHHmmss` timestamp, which is the only format DataHub accepts -- `v1` and a bare `20240610` are both rejected, despite what the upstream field documentation suggests. Changing the value updates the property in place rather than replacing it. If the name is *already* burned, adding a `version` also lets you re-create the same `property_id` on a fresh apply, which is otherwise impossible without option 2. Use a version you have not used before on that property: a versioned field is burned by an assignment just as the un-versioned one is, so re-creating at an earlier version collides again.
...
**A fixed `version` in this example would not make it re-appliable, which is why it does not set one.** The second apply would send the same version, derive the same versioned field name, and hit the same collision one level down. Only a version that changed on every cycle would work, and expressing that in HCL means deriving it from `timestamp()` -- which produces a diff on every plan whether or not anything changed. Re-appliability here is not a property a configuration can have; it belongs to the instance.
