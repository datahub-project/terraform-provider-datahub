...
| `datahub_metadata_test_validation` (data source) | The `validateTest` GraphQL query and the test engine it validates against exist only in DataHub Cloud. The `datahub_metadata_test` resource itself is OSS-compatible (the `test` entity and its mutations are `category: core` in OSS), but OSS has no engine to evaluate or validate definitions, so there is nothing for this data source to degrade to. |
| `datahub_oauth_authorization_server` (resource) | The `oauthAuthorizationServer` entity type exists only in the closed Cloud fork -- it is absent from the OSS entity registry, and the `upsertOAuthAuthorizationServer`/`deleteOAuthAuthorizationServer` mutations and `listOAuthAuthorizationServers` query are fork-only. The entity is `category: core` (not `internal`), but its only consumer is the AI plugin registry, whose configuration half lives on the `category: internal` `globalSettings` singleton -- the weakest link governs the stability posture. Note OSS contributors cannot see the backing source. See `docs/design/provider-ai-plugins.md`. |
| `datahub_data_contracts` (**data source only** -- the `datahub_data_contract` resource works on both) | The data source is backed by `searchAcrossEntities`, and `DataContractType`, the loadable GraphQL type that hydrates a search hit into an `Entity`, is registered only by the Cloud plugin and not by the OSS `GmsGraphQLEngine`. The entity itself is indexed on both (`searchGroup: primary`), so on OSS the search finds the document, has nothing to hydrate it with, and the query fails with a non-null violation on `searchResults[0].entity` rather than returning an empty list. Upstream marks the type as intended to move to OSS, so this entry has a removal condition. The resource is unaffected because its Read uses the OpenAPI v3 entity endpoint. |
| `datahub_organization_display_preferences` (resource + data source) | The `updateOrganizationDisplayPreferences` mutation, the `globalSettings` GraphQL read surface it pairs with, the `GlobalVisualSettings` PDL holding `customOrgName`/`customLogoUrl`/`primaryColor` (the last added in Cloud v2.2.0, so it is gated within a Cloud-only resource -- the provider never names it in a write until the user configures it), and the `MANAGE_ORGANIZATION_DISPLAY_PREFERENCES` privilege all exist only in the closed Cloud fork -- OSS exposes no `globalSettings` query or `updateGlobalSettings` mutation at all, so there is nothing to degrade to. The `globalSettings` entity is additionally marked `category: internal` in `entity-registry.yml`, so it carries no external API stability guarantee. Note OSS contributors cannot see the backing source. See `docs/design/provider-org-settings.md`. |
...
## Attributes newer than the resource they sit on

DataHub Cloud runs a rolling fleet, so an attribute covering a recently added DataHub feature will meet instances that predate it. Three rules keep that survivable without the provider detecting versions at all:

1. **Upgrading the provider must never break a configuration that does not use the new attribute.** This is the invariant everything else serves.
2. **Read via a path that tolerates absence.** The OpenAPI v3 entity endpoint does -- an absent field is simply missing from the JSON. A GraphQL selection set does **not**: naming a field the instance lacks fails the whole query with `Cannot query field`, breaking `terraform plan` for *every* user on that version, including those who never set the attribute. This is the only way rule 1 can be violated, and it is invisible until someone on an older instance runs a plan.
3. **Put the field on the wire only when the practitioner configured it.** For GraphQL this means absent from the variables map, not present-and-null: input-object coercion rejects an undefined field regardless of its value.

Mark such an attribute with `newInProviderNote("vX.Y.Z")` from `internal/provider/availability.go`, naming **this provider's** release, never DataHub's -- DataHub has three numbering schemes in flight with no total order across them, whereas the provider release that added an attribute is fixed at authoring time and points at the CHANGELOG entry. Delete the call once the fleet has converged; it is a one-line diff and nothing depends on it.

Failures are translated generically by `unsupportedInputField` (`internal/provider/pkg/datahub/unsupported_field.go`), which needs no per-attribute registration: the caller passes the fields that write actually carried, and the resulting `*UnsupportedFieldError` names the Terraform attribute while preserving the server's own message. Do **not** add a bespoke `isXUnsupportedError` matcher per attribute -- that is the dictionary this design exists to avoid.

`primary_color` on `datahub_organization_display_preferences` is the reference implementation, and is also the first attribute in the provider that is not owned while unconfigured (see `docs/design/provider-org-settings.md`).
...
**`govulncheck`'s own version is a `tool` directive in the root `go.mod`, not a Makefile variable.** It used to be a bare `GOVULNCHECK_VERSION ?= vX.Y.Z` -- invisible to both Dependabot (not in any `go.sum`) and the mise tool-version cadence (not a mise tool), so nothing ever proposed a bump. `go get -tool golang.org/x/vuln/cmd/govulncheck@vX.Y.Z` makes it a real dependency Dependabot's existing `gomod` block for `/` already watches. `tools/` and `tools/serve` cannot see that directive directly (separate modules, separate `go.mod`), so `scripts/vulncheck-tools.sh` derives the same version with `go -C <repo-root> list -m -f '{{.Version}}' golang.org/x/vuln` rather than taking a second, driftable copy -- one canonical pin, tracked automatically.
...
- **Two resources deliberately merge instead, and the condition that justifies it is narrow.** `datahub_structured_property_assignment` merges per property and `datahub_entity_ownership` merges per `(owner, ownership type)` pair. What earns the exception is that the aspect's list is **co-owned by design**: business users assign owners and property values through the DataHub UI on the very entities platform teams configure in Terraform, so owning the whole list would delete their work on the first apply and make the resource unusable for its primary case, which is adopting an estate somebody has already been curating. Merging is only sound where the resource can identify its own elements exactly -- state records the elements it declared, the diff runs against that, and removal *names* the element rather than rewriting the list. Where element identity is a compound key, pin every part of it: DataHub's `removeOwner` reads an omitted `ownershipTypeUrn` as "every ownership type this owner holds", so an under-specified removal silently takes more than it was asked for. Do not reach for this on a list the provider genuinely owns (policy privileges, form prompts) -- merging there makes drift invisible.
...
Current slugs, one per directory: `dataplex`, `sqlite`, `snowflake` / `snowflake-ingest` (the two connection examples), `dp`, `domain`, `pool`, `fibo`, `glossary`, `csv`, `iam`, `mdtest`, `ownership`, `azure`, `secret`, `governance`, `property`, `tag`, `homepage`, `pagetpl`, `branding`. A directory whose name already reads as the slug (`assertion-volume-sqlite` -> `tf-example-sqlite-assertion`) needs nothing more. DataHub secret names keep their SCREAMING_SNAKE convention and take the same shape uppercased: `TF_EXAMPLE_SECRET_BASIC`, `TF_EXAMPLE_AZURE_ABS_ACCOUNT_NAME`.
