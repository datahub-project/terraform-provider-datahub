...
**Access tokens, v2.2.0 note (2026-09-16).** `CreateAccessTokenInput.durationIso` and `GetAccessTokenInput.durationIso` are new, alongside the existing `AccessTokenDuration` enum (`ONE_HOUR` ... `NO_EXPIRY`). An ISO-8601 duration string replaces a fixed enum with an arbitrary lifetime, which bears directly on the open question in the access-tokens work package -- *does the WriteOnly pattern hold across a rotating credential's whole lifecycle, or only at create?* An arbitrary duration makes short-lived tokens expressible, which sharpens rather than settles it: the provider can now mint a token that expires inside a typical apply cadence, so rotation stops being hypothetical. Still no read-back of a token value, so the WriteOnly half is unchanged. Worth re-reading before that package is planned.
