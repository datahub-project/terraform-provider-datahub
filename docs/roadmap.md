...
| `createServiceAccount` / `deleteServiceAccount` + `getServiceAccount` | M/Q | **HIGH** | no (OSS 1.4.0+) | **New:** `datahub_service_account` resource. Upstreamed to OSS in DataHub Core v1.4.0 ([PR #15972](https://github.com/datahub-project/datahub/pull/15972), Jan 2026) - NOT Cloud-only. Service accounts are a `corpUser` subtype with a **server-assigned UUID** URN (`urn:li:corpuser:service_<uuid>`), so URN creation is non-deterministic. `createServiceAccount` returns only the account entity (no secret); the token is minted separately via the access-token flow (`createAccessToken`, write-once). See the note below. |
...
- **Non-deterministic URN.** A service account is a `corpUser` subtype whose URN is `urn:li:corpuser:service_<uuid>` - the server mints the UUID; there is no user-supplied id path. This is the design-doc red flag: the resource cannot derive a deterministic URN, so it must adopt the server-assigned URN as a computed value, which weakens the deterministic-URN IaC story and makes clean coexistence with SDK-created accounts harder. Resolve the URN strategy before implementing.
- **Two-aspect shape, not a single write-once create.** `createServiceAccount` returns only the account entity (urn/name/description) - no secret. The write-once credential is the *token*, minted separately via `createAccessToken` (token type = service account, shown once). So the resource is really "idempotent account + optional write-once token," not the single write-once-create originally assumed.
...
| `datahub_service_account` | server-assigned UUID (`service_<uuid>`) | **Non-deterministic** - server mints the UUID; no user-supplied id path. Adopt via import or accept a computed URN; complicates SDK coexistence. OSS 1.4.0+ (not Cloud-only). |
...
| 14 | `datahub_service_account` | resource + data source | yes (1.4.0+) | Moved from Tier 3 - OSS since Core v1.4.0. Server-assigned UUID URN (non-deterministic); token is a separate write-once step; needs Metadata Service Auth enabled |
