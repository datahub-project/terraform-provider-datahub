...
| 11 | **AI / Compass / Documents / Subscriptions** | *(mostly IRRELEVANT)* | none | Runtime/per-user/experimental Cloud features. **Carve-out (2026-07-30):** the tenant-level AI plugin registry is real admin-owned config — see Tier 3 items 15 and 18, and [provider-ai-plugins.md](design/provider-ai-plugins.md). |
...
| `moveDataProduct` + `parentDataProduct` + `getRootDataProducts` | M/Q | **HIGH** | verify | **New at v2.2.0 (2026-09-16). A gap in a shipped resource, and the best-shaped increment in the release.** Data products became hierarchical: `MoveDataProductInput.parentDataProduct`, `CreateDataProductPropertiesInput.parentDataProduct`, `UpdateDataProductInput.parentDataProduct`, `DataProduct.parentDataProducts`, plus a `getRootDataProducts` query. This is structurally identical to the domain hierarchy `datahub_domain` already ships -- `moveDomain` mapped to a `parent_domain` attribute -- so the design is precedent, not new pattern work: add `parent_data_product`, reparent in place via `moveDataProduct`. Until it is modelled, a hierarchy built in the UI is invisible to the provider, and since `datahub_data_product` writes `dataProductProperties` wholesale via OpenAPI v3 it may be **silently flattened on the next apply** -- the list-ownership trap, on a resource users already run. Rate as HIGH once the OSS/Cloud split is confirmed; `nestedDomainsEnabled` has an OSS precedent, so this probably follows. |
...
| `batchAddOwners` / `removeOwner` (on config entities) | M | **covered** | no | `datahub_entity_ownership` resource: one resource per owned entity, holding a set of `(owner, ownership type)` pairs. **Not a deny-list exception** -- the Category 8 Caveat has always said that setting `ownership` on a config entity the provider manages is legitimate, and this row is that sentence implemented. Accepted targets are the eight entity types that both carry the `ownership` aspect and sit on the config side of the line: `domain`, `glossaryTerm`, `glossaryNode`, `dataProduct`, `corpGroup`, `tag`, `form`, `dataHubIngestionSource`. Data assets are rejected at plan time, and so are `corpuser`, `dataContract`, `dataHubPolicy` and `structuredProperty` -- for the opposite reason, since those declare no `ownership` aspect at all and a write would land nowhere (three of them *are* valid `datahub_structured_property_assignment` targets, so the two allowlists deliberately differ). **Merge semantics**, departing from the provider's usual own-the-whole-list rule: the resource owns only the pairs it declares, so adopting a glossary that already has UI-assigned owners does not delete them. `(entity, ownership type)` is not a unique key in either direction -- several owners share one type and one owner holds several, both routinely. Writes go through GraphQL rather than an aspect write specifically to get `OwnerUtils.validateOwners` owner-existence checking; `removeOwner` always pins `ownershipTypeUrn`, because omitting it removes the owner under every type it holds. |
...
| `upsertOAuthAuthorizationServer` / `deleteOAuthAuthorizationServer` + `oauthAuthorizationServer(urn)` | M/Q | **HIGH** | yes | **New:** `datahub_oauth_authorization_server` resource. Config for external OAuth IdPs. Cleanest "config" object in the access space. **Hard prerequisite for `datahub_ai_plugin` (Tier 3 item 18):** the plugin's `USER_OAUTH` mode references it via `oauthServerUrn`, and the only alternative — the plugin mutation's inline `newOAuthServer` input — refuses a caller-supplied id and always mints a UUID, which the URN-determinism rule forbids. Build first, release together (a standalone OAuth server has no consumer in DataHub today, so shipped alone it changes nothing observable). The entity is fork-only (absent from the OSS entity registry), though `category: core` rather than `internal`. Design: [provider-ai-plugins.md](design/provider-ai-plugins.md). |
...
| `createAccessToken` / `revokeAccessToken` + `getAccessToken` | M/Q | MEDIUM | no | Write-once tokens with TTL; rotation = constant churn. Skip unless customers ask. Server policy caps the TTL far more tightly than the enum suggests -- `NO_EXPIRY` is refused by default and one year is the ceiling; see the access-tokens note below before planning this. `revokeAccessToken` is a genuine revocation (a revoked token returns 401), not just a metadata delete. |
...
| `corpUser` lookup by email address | Q | **LOW -- deferred, blocking nothing** | no | `datahub_corp_user` (data source) looks a user up by `username`, which is the URN suffix. Where the username is not the email address -- SSO deployments where it is an opaque id, or a DataHub whose users predate a naming convention -- a practitioner writing `datahub_entity_ownership` has to know the URN suffix already. An `email` lookup argument would close that, backed by `searchAcrossEntities` on `corpUserInfo.email` (OpenSearch, so it would carry the usual lag caveat for a data source and could not be used by any Read path). **Explicitly not a blocker for `datahub_entity_ownership`:** an owner URN is an ordinary string attribute, so a raw URN, a variable or another resource's `.urn` all work today, and the common case is that the username *is* the email address. Recorded so the gap is a known one rather than a surprise. |
| `datahub_corp_users` plural data source | Q | **LOW -- deferred, blocking nothing** | no | `corp_user` is the only identity type in the provider with no plural enumerator: `corp_group`, `role`, `ownership_type`, `service_account` and `secret` all have one. `listUsers` exists and the importtarget registry already enumerates corpUser URNs for bulk import, so the data is reachable -- there is simply no `data.datahub_corp_users`. Nothing is blocked by its absence, including `datahub_entity_ownership`, whose owner URNs are plain strings. Worth doing for symmetry and for the `for_each` patterns the other plural sources enable, not for any capability it unlocks. |
...
**Access tokens, v2.2.0 note (2026-09-16), corrected by measurement 2026-09-18.** `CreateAccessTokenInput.durationIso` and `GetAccessTokenInput.durationIso` are new, alongside the existing `AccessTokenDuration` enum (`ONE_HOUR` ... `NO_EXPIRY`). **The original reading of that -- an ISO-8601 string replacing the enum with an arbitrary lifetime, so the provider could mint a token expiring inside a typical apply cadence -- is wrong.** It was inferred from the schema and never checked against a server. A live probe against DataHub Cloud v2.2.0 on 2026-09-18 found `durationIso` gated by an allowlist, so it carries no more expressive power than the enum unless an operator widens that allowlist:

| Request | Result |
|---|---|
| `duration: NO_EXPIRY` | refused -- `ACCESS_TOKEN_ALLOW_NO_EXPIRY is false` |
| `duration: ONE_HOUR` / `ONE_YEAR` | minted |
| `durationIso: PT1H` / `P365D` / `P1Y` | minted (`P1Y` accepted as equal to `P365D`) |
| `durationIso: PT15M` / `P100Y` | refused -- not in `ACCESS_TOKEN_ALLOWED_DURATIONS` |
| both `duration` and `durationIso` | refused -- "exactly one of" |
| neither | refused -- there is no default |

**The gate is OSS, and its default is restrictive.** `AccessTokenDurationPolicy.resolveExpiresInMs` (`datahub-graphql-core/.../resolvers/auth/AccessTokenDurationPolicy.java`) enforces XOR of the two fields, allowlist membership **by millisecond value rather than string** (hence `P1Y` == `P365D`), and the never-expire gate. The defaults live in `metadata-service/configuration/src/main/resources/application.yaml`: `allowNoExpiry: ${ACCESS_TOKEN_ALLOW_NO_EXPIRY:false}` and `allowedDurations: ${ACCESS_TOKEN_ALLOWED_DURATIONS:PT1H,P1D,P7D,P30D,P90D,P180D,P365D}`. So out of the box **a never-expiring token cannot be created at all, one hour is the shortest and one year the longest** -- on OSS and Cloud alike, not a demo-instance lockdown. The enum still advertises `NO_EXPIRY`, which is why introspection alone looks permissive; the refusal is a runtime policy layer invisible to the schema. The gate is create-time only, and its own error message says so: existing never-expiring tokens keep working, so an instance can hold immortal tokens minted before the flag was set.

**Two consequences for the access-tokens work package, when it is planned.**

- **Rotation is less acute than the original note claimed, so the WriteOnly question is unchanged rather than sharpened.** The shortest mintable token lasts an hour, which comfortably outlives a single apply -- the case the original note worried about. It does not outlive the gap *between* applies, so a Terraform-managed token still expires in state and rotation remains a real design question; it is simply not a new one raised by `durationIso`. There is still no read-back of a token value, so the WriteOnly half is untouched either way. Design for the one-year ceiling as the normal case.
- **The policy is readable at plan time, which is what makes a good diagnostic possible.** `appConfig.authConfig` exposes `tokenAuthEnabled`, `allowNoExpiry` and `allowedAccessTokenDurations` (`datahub-graphql-core/src/main/resources/app.graphql`). A `datahub_access_token` resource should validate a requested duration against that during plan, because the server-side refusal arrives as a GraphQL error inside an **HTTP 200**, which is exactly the shape that reads as success to a careless client.

**Re-probe condition:** the note above describes default config, so any instance may differ. One query answers it, and it needs no write: `{ appConfig { authConfig { allowNoExpiry allowedAccessTokenDurations } } }`.
...
| `submitTestRun` / `testRun` / `testrunstatus` aspect | M/Q | **LOW** | yes | **New at v2.2.0.** `submitTestRun` mutation, `testRun` query, `TestRun` / `TestRunHandle` / `TestRunState` (`PENDING`/`RUNNING`/`COMPLETE`/`FAILED`) and `RunTestResult` (`passingCount`, `failingCount`, `tokensConsumed`, `toolCallsUsed`), plus `/openapi/v3/entity/test/{urn}/testrunstatus`. Rated LOW on the same ground as `runTests` above: **submitting a run is operational, not declarative**, and a resource that triggers execution on apply would make `terraform plan` non-idempotent. It matters to the metadata-tests work package anyway, because it changes what the package is *for* -- the token and tool-call counters show runs are now LLM-backed, so a test definition has a per-run cost. That is an argument for landing `datahub_metadata_test` with `validateTest` plan-time validation first and leaving execution to the platform. |
...
| AI settings (`aiAssistant`, `documentationAi`, `aiContext`, `mcpSettings`) | M/Q | **HIGH** | yes | **New:** one `datahub_ai_settings` covering all four — one persona, one page, one tier. `aiPlugins`/`mcpServers` are object lists with independent lifecycles, so they are child resources (e.g. `datahub_ai_plugin`), not attributes. Per-user siblings (`updateCorpUserAiAssistantSettings`, `updateUserAiPluginSettings`, `updateUserContextDocumentsSettings`) are out of scope. `datahub_ai_plugin` is being built ahead of `datahub_ai_settings` ([provider-ai-plugins.md](design/provider-ai-plugins.md)). Constraint that falls out: `datahub_ai_settings` must write via `updateGlobalSettings` (server-side read-modify-write) and must **never** write `globalSettingsInfo` directly via OpenAPI v3 — a partial aspect write would clobber `aiPluginSettings.plugins`, silently deleting every AI plugin registration in the tenant. |
...
| ~~Brand colour (`primaryColor`)~~ | M/Q | **HIGH** | yes | **Shipped**: `primary_color` on `datahub_organization_display_preferences` (resource + data source). It is the first attribute in the provider that is *not* owned while unconfigured -- full ownership would have named `primaryColor` in every user's mutation, which a Cloud instance older than v2.2.0 rejects outright, turning an optional attribute into a breaking change; see [provider-org-settings.md](design/provider-org-settings.md). ~~**BLOCKED until the Cloud release ships and reaches demo.acryl.io.**~~ **UNBLOCKED at v2.2.0 (2026-09-16) -- landed, and every question this row was waiting on is answered.** `GlobalVisualSettings.primaryColor` and `UpdateOrganizationDisplayPreferencesInput.primaryColor` are both live on demo.acryl.io (absent at v2.1.0, so it arrived in this release). The three unknowns resolve as: the mutation is `updateOrganizationDisplayPreferences`, already the shipped resource's own write path, so there is no new transport to build; the value does sit in `globalSettingsInfo.visual` directly alongside `customOrgName`/`customLogoUrl`, confirming the domain grouping; and the format is a hex `String` (`#EC0016`), with **empty string as the documented clear-to-default sentinel** -- worth modelling deliberately, since `null` (attribute absent) and `""` (explicitly restore the default brand) are different intents and the provider owns what it declares. Cloud-only: no `primaryColor`, and no org-display-preferences surface at all, in the OSS GraphQL tree. Still **one more attribute on `datahub_organization_display_preferences`**, not a `datahub_brand_color`. |
...
| UI dark mode (`themeDarkModeEnabled`) | flag | **IRRELEVANT** | no (OSS) | **Not provider-actionable, and the reason is structural rather than a matter of timing.** Dark mode has two halves and neither is an API surface. The server half is `themeDarkModeEnabled`, a `FeatureFlags` entry set by the `THEME_DARK_MODE_ENABLED` environment variable and defaulting to `false` -- deployment configuration that belongs to a Helm chart, not to this provider, and read-only over GraphQL like every other `FeatureFlagsConfig` field. The user half is a browser `localStorage` key (`isDarkModeEnabled`, see `useIsDarkMode.ts`), which never reaches the server, so there is no aspect to write and it would be a per-user preference excluded by the scope rule even if there were. Turning the flag on merely reveals the toggle in Appearance settings. Status as of 2026-09-16: **in OSS master, not yet in Cloud** -- `themeDarkModeEnabled` is absent from the v2.2.0 introspection, matching the internal call for OSS feedback before enabling it for SaaS in a later release. Distinct from the older `themeV2*` flags, which are the V2 redesign and have been present since at least v2.1.0. |
...
**Owner assignments:** `addOwner`, `addOwners`, `batchAddOwners`, `removeOwner`, `batchRemoveOwners` -- *on config entities the provider manages, the first two are shipped.* `datahub_entity_ownership` covers `batchAddOwners`/`removeOwner` for those; everything here remains deny-listed on data assets. See the Caveat below and the Category 2 row.
...
**Two of the three are now built on the strength of that Caveat**, which is worth recording because the sentence sat here for months reading like a hypothetical. `upsertStructuredProperties` became `datahub_structured_property_assignment` (Category 3), and `batchAddOwners` / `removeOwner` became `datahub_entity_ownership` (Category 2). Both enforce the split in code -- an explicit allowlist of config entity types, with data assets refused at plan time -- rather than relying on the practitioner having read this section. `domains` on a config entity remains the unbuilt third, reachable today only as the `domain` attribute on the individual resources that carry it.
...
- ~~`upsertAiPlugin`/`deleteAiPlugin`~~ — **promoted 2026-07-30** to `datahub_ai_plugin`, Tier 3 item 18. Previously filed here as "borderline; revisit if customers ask". The ask arrived: an internal demo hand-scripts `upsertOAuthAuthorizationServer` + `upsertAiPlugin` because the provider cannot do it. The original bucketing was wrong on the merits, not just on demand — unlike its neighbours here, this is neither runtime nor per-user state. It is tenant-level, admin-owned, slow-moving registry config (a `service` entity plus one entry in `globalSettingsInfo.aiPluginSettings.plugins`), gated by one privilege (`MANAGE_CONNECTIONS`). The per-user siblings (`updateUserAiPluginSettings`, the per-user OAuth Connect flow) correctly stay in this bucket. Both mutations were **confirmed live** on demo.acryl.io v2.0.3 (2026-08-02) rather than assumed present. Hard dependency on item 15. Design: [provider-ai-plugins.md](design/provider-ai-plugins.md)
...
- **Cloud-experimental:** file only in fork, no OSS equivalent — includes `agents`, `aimemory`, `evals`, `share`, `draft`, `actions`, `actions_pipeline`, `ai`, `constraints`, `versioning.glossary`
- **`aiPlugin` — corrected 2026-07-30.** Split verdict, previously listed above as having no OSS equivalent. The GraphQL surface (`aiPlugin.graphql`, the OAuth mutations in `service.graphql`, the `AiPluginConfig`/`AiPluginSettings` PDL) is fork-only, so both resources are **Cloud-only in practice**. But the `service` entity type has been upstreamed to OSS `entity-registry.yml` as `category: core` (verified against OSS v1.6.0rc1: `keyAspect: serviceKey`), its `ServiceKey.pdl` is in OSS, and `ServiceUrn` ships in the OSS Python SDK. `oauthAuthorizationServer` really is fork-only end to end. Both entity types are `category: core`, not `internal` — but the AI plugin's config half lives on the `category: internal` `globalSettings` singleton, so the weakest link governs the stability posture.
...
| `datahub_oauth_authorization_server` | user-supplied `id` -> `urn:li:oauthAuthorizationServer:<id>` | Cloud-only. No OSS SDK URN class exists for this type, so there is no competing convention — the format is exactly what the mutation constructs. Omitting `id` mints a UUID, so the provider always sends it; the plugin mutation's inline `newOAuthServer` path is unusable (it refuses an id). |
| `datahub_ai_plugin` | user-supplied `id` -> `urn:li:service:<id>` | confirmed — matches the OSS SDK's `ServiceUrn(id)` (single-part, same key aspect). Omitting `id` mints a UUID; the provider always sends it. Validate the id against the SDK's URN-reserved characters (comma, parentheses, U+241F). |
...
- `datahub_oauth_authorization_server` (`scopes`, `additional_token_params`, `additional_auth_params`) — note the mutation's per-field null semantics are **not uniform**: some fields preserve on null and some clear, so the provider must send every field it exposes on every write. Table in [provider-ai-plugins.md](design/provider-ai-plugins.md).
- `datahub_ai_plugin` (`custom_headers`, `required_scopes`) — `required_scopes` may not be clearable at all (an empty list appears to preserve rather than clear). Open question; if confirmed, reject `[]` at plan time rather than accepting a config the provider cannot honour.
...
Add `service` (for `datahub_ai_plugin`) and `oauthauthorizationserver`. Both are structurally registered rather than per-type: the v3 entity controller is generic over the entity registry and `getEntitySpec` lower-cases its argument, which is the same mechanism that makes `globalsettings` work. The residual risk is therefore not "is the endpoint registered" but (a) the deployed Cloud build's registry contents and (b) the per-entity-type API read authorization — neither entity carries `viewUnrestricted: true`. Probe both before implementing; see [provider-ai-plugins.md](design/provider-ai-plugins.md) open question 2. `datahub_ai_plugin` additionally reads `globalsettings` (already proven live) to recover the half of its state that lives on the singleton.
...
**The read path is what decides whether rule 1 holds, and it is easy to get wrong silently.** Point 2 of the default shape above says "treat a missing aspect as null", which is free over the OpenAPI v3 entity endpoint -- an absent field is simply not in the JSON. It is **not** free over GraphQL: a selection set naming a field the instance lacks fails the *entire* query with `Cannot query field`, so Read and therefore `terraform plan` break for every user on that version, including everyone who never touched the attribute. That is the one way a provider upgrade alone can break a working configuration, and nothing in the schema or the tests reveals it -- only a plan against an older instance does. `primary_color` is safe because `datahub_organization_display_preferences` reads over OpenAPI v3; an attribute whose read is GraphQL needs the field omitted from the selection set, or a different read path. Check this before adding an attribute to a shipped resource, not after.

**Diagnostics are generic, not per-attribute.** `unsupportedInputField` (`internal/provider/pkg/datahub/unsupported_field.go`) recognises a rejection naming an input field the write actually carried and returns an `*UnsupportedFieldError` naming the Terraform attribute, with the server's message preserved. It takes the candidate fields from the caller rather than a central table, so a new attribute gets a usable diagnostic without registering anything. This replaces the per-feature `isXCloudOnlyError` pattern for the version-skew case: eight of those already exist for the Cloud-only case, and one per future attribute does not scale.
...
| 15 | `datahub_oauth_authorization_server` | resource + data source | yes | Stable config object. **Build before item 18** (hard prerequisite via `oauthServerUrn`), **release together** — no consumer exists without the AI plugin. Carries the WriteOnly client-secret design. |
| 18 | `datahub_ai_plugin` | resource + data source | yes | Promoted from Category 11 (2026-07-30) on an internal demand signal. Depends on item 15. Half its state lives in a list inside the `globalSettings` singleton aspect — a new shape for this provider — so Read is two OpenAPI v3 calls, writes need serialising behind `keyedMutex` on the singleton, and both delete paths cascade into the other resource. Design: [provider-ai-plugins.md](design/provider-ai-plugins.md). |
...
**Re-surveyed again 2026-09-16** against demo.acryl.io upgraded to **DataHub Cloud v2.2.0** (commit `3902d4d09c4dbae8b8f15b7f09b823e10d9c063b`). The first survey run as a true two-file diff rather than a reconstruction, using the archive created after the last one:

| | v2.1.0 | v2.2.0 | Change |
|---|---|---|---|
| v3 paths | 1,159 | 1,198 | +39 |
| GraphQL queries / mutations | 176 / 298 | 181 / 303 | +5 / +5 |
| GraphQL types | 1,646 | 1,686 | +40 |
| Entity types at `/openapi/v3/entity/` | 93 | 93 | 0 |
| Removals | | | 3, all `FeatureFlagsConfig` fields |

Far calmer than v2.1.0's +152 paths / +704 operations, and unusually it produced a **HIGH** finding in an area already shipped rather than a new entity type: brand colour (Category 7) and hierarchical data products (Category 2). The three removals are `hideChatBetaLabel`, `lineageGraphV2` and `lineageGraphV3` -- feature flags, no provider impact.

Also new and out of scope, listed so nothing is silently dropped: an `aiJudgmentResultCache` aspect added to roughly 20 entity types at once (derived AI cache, same shape as the `semanticContent` note below), a new `eval` entity family with ~15 `Eval*` types, five `/openapi/operations/semantic-search/*` endpoints, `proposeCreateDomain` / `proposeCreateDataProduct` / `proposeDataContract` (proposal workflow, runtime), `upsertIncident`, and `searchAcrossLineageCounts`. Two worth a second look when their categories are next touched: `globaltags` on `glossaryNode` and `glossaryTerm` (platform-config entities, so *not* caught by the per-asset deny-list) and `applications` on `glossaryNode`.

**Two method traps surfaced, both now fixed in the tooling, and either would have produced a false survey:**

- **Springdoc group ids are not stable.** v2.2.0 renumbered them -- `openapi-v3` became `10-openapi-v3` -- and relabelled the v2 group *"DataHub v2 (OpenAPI, deprecated)"*. Matching derived files by filename would have reported the entire v3 surface as removed and re-added, roughly 2,400 phantom changes burying the 39 real ones. The diff tool now normalises the numeric prefix before matching.
- **A derived index is only comparable to one built the same way.** The first run reported 1,032 removed GraphQL fields. Every one was an artefact of the v2.1.0 index having been built by an older script that included enum values and `__` introspection meta-types. Re-deriving both sides from archived raw JSON under one method gave the real answer, 3. The rule: **when the derivation changes, re-derive every snapshot before diffing** -- which only works because the raw documents are archived, and is the clearest vindication so far of archiving them.
...
5. **Re-probe 2026-09-16 (v2.2.0)** — recorded so a future reader does not re-litigate them. **Changed:** the brand-colour field has **arrived** (`GlobalVisualSettings.primaryColor`), retiring the longest-standing BLOCKED note in this document. **Unchanged:** `ActorFilterInput` still has no `roles` field, so the unconditional policy write guard stands (CAT-3192); still no top-level `dataContract` query, so reads stay on `entity()` or OpenAPI v3 (`upsertDataContract` and the new `proposeDataContract` are the only mutations); `relationshipType` remains readable but unwritable. One correction while re-probing: this document records that "`RelationshipType` introspects as null", but the deployed type is named **`RelationshipTypeEntity`** and has been present since at least v2.1.0. The conclusion is unaffected — there are still no write mutations, so it is still not buildable under the write convention — but the stated evidence was wrong, and a future probe looking up `RelationshipType` would keep finding null and keep drawing the right conclusion for the wrong reason.
