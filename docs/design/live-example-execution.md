...
**Status: fully implemented.** `internal/provider/example_live_test.go` holds the harness, `internal/provider/example_live_classification_test.go` the run list and the completeness check, `.github/workflows/live-examples.yml` the CI job, and `make test-examples-live` runs it locally. **Eighteen of the twenty-six runnable examples are applied live -- every one that can run against an OSS Quickstart and leave it as it found it.** The remaining eight are excluded permanently (five Cloud-only, two on cost, one on shape); no deferred exclusion remains. Sections below that describe a decision still read as a design; where implementation changed one, the section says so.

**Every count in this document comes from the harness, not from counting directories.** `TestEveryRunnableExampleIsClassified` logs a one-line summary, so the authoritative figure is whatever `mise exec -- go test ./internal/provider/ -run TestEveryRunnableExampleIsClassified -v` prints. At the time of writing that is `26 runnable examples: 18 applied live, 8 excluded (8 permanently, 0 deferred)`. Where a section below reports a *measurement* rather than a count, it keeps the run list it was measured against and says which -- a timing taken over sixteen examples is not improved by renumbering it to eighteen.
...
## Scope: all 26 runnable examples classified
...
**This table is maintained in step with the tree, and the reason is worth stating because an earlier revision argued the opposite.** It listed 19 directories when the design was written; two more arrived with #116, and that revision left the count stale deliberately, on the grounds that the drift "is exactly what `TestEveryRunnableExampleIsClassified` exists to catch". That was half right, and the wrong half is the half that mattered. The completeness test guards `liveExamples` and `liveExampleExclusions` -- the Go tables the harness actually runs from -- and it does so absolutely: an unclassified directory fails a plain `go test ./...`, which is why no example has been unclassified since. **It does not guard this file.** Nothing does. So the stale count here demonstrated no gap in the harness; it only made the one document a reader consults to understand the harness wrong about its own scope, which teaches the reader a false fact rather than a true lesson. The lesson worth keeping is the one in the paragraph above: derive the counts from the test's own summary line, because that is the only figure the build maintains for you.
...
| `data-contract-with-ingestion` | Poor fit by shape | `datahub_data_contract` (`main.tf:112`) and `datahub_custom_assertion` (`main.tf:103`) are both `ossAndCloudBadge` (`data_contract_resource.go:91`, `custom_assertion_resource.go:85`), so this is the only exclusion owing nothing to Cloud or cost. It triggers an ingestion *run* through a `local-exec` provisioner (`main.tf:62-98`), because running a source is a verb and the provider exposes no resource for it. The dataset that run produces is unmanaged, so destroy leaves it behind with no `tf-example-` identifier for a sweeper to attribute it by -- and because it survives, the re-apply check would run against a different world than the first apply, quietly weakening the strongest assertion the harness has | Needs `curl`, `bash` and a live ingestion executor. Its value is documentation: a contract needs a dataset and a dataset comes from ingestion, which the acceptance test cannot show because that writes the aspect directly and no user obtains a dataset that way |
...
| `form-simple` | Runs on Quickstart | Two `datahub_form` resources (`main.tf:79`, `main.tf:138`) -- one `COMPLETION`, one `VERIFICATION` -- plus the two structured properties their prompts collect and a stewardship group, all `ossAndCloudBadge` (`form_resource.go:113`) | None, and two hazards it looks like it should have are absent. It *defines* structured properties and assigns them to nothing, so it burns no Elasticsearch field (see "Flakiness" item 8) and needs neither `settleAfterDestroy` nor a `noReapplyReason`. Its dynamic assignment matches postgres and mysql datasets, of which a Quickstart has none, so the asynchronous assignment hook has nothing to assign and the destroy nothing to retract |
...
| `metadata-test-simple` | Runs on Quickstart | Two `datahub_metadata_test` resources (`main.tf:34`, `main.tf:62`) plus the plural data source (`main.tf:94`); the `test` entity and its mutations are `category: core` in OSS and the resource is `ossAndCloudBadge` (`metadata_test_resource.go:62`) | Only Cloud *evaluates* a test, and a stock OSS UI does not even display one (`testsConfig.enabled` defaults to false), so what a live OSS run covers is the CRUD surface -- which is exactly the half a live run is for. `data.datahub_metadata_tests.all` is `listTests`-backed and index-lagged, the same false-failure class `tag-simple` documents. The Cloud-only `datahub_metadata_test_validation` data source is deliberately not used here |
| `oauth-authorization-server-simple` | Cloud-only | `datahub_oauth_authorization_server` (`main.tf:21`) is `cloudOnlyBadge` (`oauth_authorization_server_resource.go:80`); the `oauthAuthorizationServer` entity type is absent from the OSS entity registry | Two variables have no default (`variables.tf:1,7`), the second a WriteOnly client secret, so Terraform 1.11 or later is required |
| `org-branding-simple` | Cloud-only | `datahub_organization_display_preferences` (`main.tf:42`) is `cloudOnlyBadge` (`organization_display_preferences_resource.go:87`); OSS exposes no `globalSettings` GraphQL read or write surface at all | **The only example that would still be excluded on a Cloud target**, and worth reading for the reason rather than the verdict. Every other example creates entities under its own `tf-example-` ids, so applying it affects nothing that was already there and destroy puts the instance back. This one writes the shared, always-present `globalSettings` singleton that every user of the instance sees, and destroy resets those fields to DataHub's defaults rather than to whatever the instance had -- so the harness could not leave the target as it found it even in principle. The example's own README carries the same warning |
...
Totals: **18 run on Quickstart**, **5 are Cloud-only**, **2 are too expensive or slow**, **1 is a poor fit by shape**.

**All 18 now run.** The first slice ran 6 -- `provider-install-verification` (preflight), `tag-simple` (index-lagged plural data source), `domain-simple` (child-delete race), `structured-and-custom-properties` (worst CAT-2583 shape), `page-template-simple` (the boring control) and `home-page-layout` (restore-on-destroy) -- chosen to cover both known flake classes and the assertion inversion rather than to maximise count, because until it ran every wall-clock figure below was an estimate. The other ten were marked `deferred` rather than permanently excluded so the expansion would not have to re-derive which was which, and that is exactly how the expansion proceeded. `form-simple` and `metadata-test-simple` arrived afterwards and joined the run list in the commits that added them, which is the completeness test working as designed: neither could have merged unclassified. `liveExampleExclusions` holds eight permanent entries and nothing else.

Two observations about the exclusion buckets survive the growth, and one bucket is new.

The fourth designed bucket, **needs-external-credentials, is still empty as a primary verdict**, and that is a finding rather than an oversight. Every example that needs a real third-party credential is already excluded by a stronger reason: `remote-executor-azure` on cost, `action-pipeline-dataplex-sync` on Cloud-only, `financial-services` on cost and Cloud-only, `oauth-authorization-server-simple` on Cloud-only. The two Snowflake examples look like members of the bucket but are not: DataHub stores the connection blob without ever dialling Snowflake, so synthetic values apply and destroy exactly like real ones.

**Poor fit by shape is a fifth bucket the design did not anticipate, and `data-contract-with-ingestion` is its only member.** It is worth naming separately rather than filing under a stretched reading of one of the others, because it is the only exclusion that would survive a Cloud-capable target *and* an unlimited budget. What disqualifies it is that part of what it demonstrates is not Terraform at all -- a `local-exec` provisioner drives an ingestion run, and the dataset that run produces is outside Terraform's management, so destroy cannot remove it and the next apply does not start from the same instance state. An example the harness cannot return to a known-empty target breaks the destroy-leaves-nothing assertion by construction rather than by failing it. The neighbouring case is `org-branding-simple`, excluded on Cloud-only but for substantially this reason too: both write state the harness does not own.
...
The run list has since grown to eighteen, and this table is left at the sixteen it was measured over rather than extended with figures nobody took. `form-simple` and `metadata-test-simple` are both in the shape the table shows costing about a second -- a handful of entity writes, no settle, no phase -- so the expected whole-job effect is on the order of two seconds, but that is a prediction and the point of this table is that it contains none.
