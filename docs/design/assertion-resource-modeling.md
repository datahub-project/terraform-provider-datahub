...
Note on type-routed enumerators: a `source==NATIVE` + per-type enumerator that *skips* shapes a typed resource cannot model was prototyped (branch `assertion-type-routed-enumerators`) but is **not** the recommended end state -- skipping leaves NATIVE assertions unmigratable (violates "all NATIVE importable"), and splitting one assertion type across two resources violates "one consistent target". The generic resource (2) is what makes NATIVE assertions completely and consistently migratable.
