...
**Measured 2026-09-18, and it constrains how short "short-lived" can be.** A DataHub access token's lifetime is not free-form: `AccessTokenDurationPolicy` validates the requested duration against `ACCESS_TOKEN_ALLOWED_DURATIONS`, whose default is `PT1H,P1D,P7D,P30D,P90D,P180D,P365D`, so **one hour is the shortest token an operator can mint without changing server configuration**, and `PT15M` is refused outright. The v2.2.0 `durationIso` field does not change this -- it is checked against the same allowlist by millisecond value. A one-hour floor is still a long-lived credential by the standards the paragraph above is measuring against, which strengthens rather than weakens the case for the configuration layer being explicit. Details and the re-probe query are in [roadmap.md](../roadmap.md), under the access-tokens note in Category 4.
