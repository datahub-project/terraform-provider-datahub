...
- [x] Structured property upsert on the three new assignment target types - verified 2026-07-23 against DataHub Cloud (demo.acryl.io): corpuser, corpgroup, and datacontract paths all accept upsertStructuredProperties, persist, and read back exactly (assignment lifecycle incl. in-place update and import on corpGroup); SP defaults latch lifecycle, entityTypes filtering, and assignment coexistence also verified live on the same run. One mock/live divergence found: the live upsertDataContract validates referenced assertion URNs exist (mock does not) - scenarios create a real custom assertion.
...
| 3 | `globalTags` client + `defaults.tags` on corp_user/service_account/corp_group/data_product (`tags_all` ownership latch, tag-existence guard, read-back verified writes) | shipped (#88) |
| 4 | Tag defaults on the 6 assertion resources (`tags_all` latch reuse; assertion entity path live-verified 2026-07-23 - the read-back write guard turns a CAT-2562-style silent no-op into an explicit error) | shipped (#91) |
| 5 | Assignment-target extension (corpuser, corpGroup, dataContract). Registry short name for users is all-lowercase `corpuser`. | shipped (#92) |
| 6 | Structured-property defaults (`structured_properties_defaults`, per-property ownership latch; definitions prefetched at Configure with warn-and-skip on missing so destroy is never blocked; apply-time re-check hard-errors on writes) | shipped (#92) |
| 7 | Docs guide (`guides/provider-defaults`), provider example, structured-and-custom-properties README note, roadmap note | shipped (#92) |
