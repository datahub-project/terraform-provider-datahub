...
- **CLI path** -- `datahub-tf-export enumerate` exports your DataHub resources as Terraform configuration automatically. Recommended for brownfield deployments with more than a handful of resources. You then run `terraform apply` on the output to complete the import into Terraform state.
...
- `datahub-tf-export` binary (see installation options below)
- `DATAHUB_GMS_URL` and `DATAHUB_GMS_TOKEN` set in your environment

#### Installing datahub-tf-export
...
```toml
[tools]
"ubi:datahub-project/terraform-provider-datahub" = { version = "0.2.0", exe = "datahub-tf-export", matching = "datahub-tf-export" }
```
...
Download the `datahub-tf-export_<version>_<os>_<arch>.zip` archive for your platform from the [GitHub releases page](https://github.com/datahub-project/terraform-provider-datahub/releases), unzip it, and move the binary to a directory on your PATH:

```shell
unzip datahub-tf-export_0.2.0_darwin_arm64.zip
mv datahub-tf-export_v0.2.0 /usr/local/bin/datahub-tf-export
```
...
make install
# binary written to ./bin/datahub-tf-export
```

### Step 1: enumerate and generate

```shell
datahub-tf-export enumerate --output ./import
```
...
```shell
datahub-tf-export enumerate --output ./import --types datahub_secret,datahub_connection
```

To write `import.tf` only without running terraform (useful for inspecting URN enumeration):

```shell
datahub-tf-export enumerate --output ./import --skip-terraform
```
...
The `datahub-tf-export` CLI appends a commented stub listing all available platform block types to each generated connection resource.
...
Cloud-only. The `datahub-tf-export` CLI skips remote executor pools when run against OSS DataHub and prints a notice. On DataHub Cloud, pools import cleanly with no write-only attributes.
