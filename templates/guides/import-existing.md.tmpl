...
```toml
[tools]
"ubi:datahub-project/terraform-provider-datahub" = { version = "0.8.0", exe = "datahub-tf-extract", matching = "tools-datahub-tf-extract" }
```
...
```shell
unzip tools-datahub-tf-extract_0.8.0_darwin_arm64.zip
mv datahub-tf-extract_v0.8.0 /usr/local/bin/datahub-tf-extract
```
